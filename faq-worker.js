/**
 * Cloudflare Worker — FAQ chatbot backend
 * ----------------------------------------
 * This holds your Anthropic API key server-side and answers questions
 * grounded ONLY in the FAQ_KNOWLEDGE_BASE below. Deploy this once;
 * point your widget's WORKER_URL at it.
 *
 * SETUP (one-time, ~10 minutes):
 * 1. Go to https://dash.cloudflare.com -> Workers & Pages -> Create -> Worker
 * 2. Delete the default code, paste this whole file in, click Deploy.
 * 3. Go to Settings -> Variables -> add an ENCRYPTED variable:
 *      name:  ANTHROPIC_API_KEY
 *      value: <your key from console.anthropic.com>
 * 4. Copy the worker's URL (looks like https://faq-worker.YOURNAME.workers.dev)
 *    and paste it into WORKER_URL in the widget HTML file.
 * 5. (Optional but recommended) In Settings -> Variables, add ALLOWED_ORIGIN
 *    set to your site's URL (e.g. https://yourname.github.io) to restrict
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // Build conversation: prior turns (if any) + new user message
      const messages = [
